...
def test_complete_todo_dsl():
    ...
    #browser.all(x.all().by(its.have_id('todo-list')).child('li').by(its.without_class('completed')).build())
    # .should(have.exact_texts('a', 'c'))
    browser.all(x.all().by(its.have_id('todo-list')).child('li').by(its.by_not(its.with_class('completed'))).build())\
        .should(have.exact_texts('a', 'c'))
